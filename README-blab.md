...
| `fix/sbs-partially-diffusion-controlled` | `TsEmDNAChemistry` never read the per-reaction `ReactionType` the decks set, so every step-by-step reaction ran fully diffusion controlled while the IRT path honoured the declaration. Maps deck types 2 and 4 onto Geant4's partially diffusion controlled treatment and type 5 onto its spin-statistical probability. A reaction whose rate and radii are inconsistent falls back with a loud report rather than aborting, because the shipped IRT reaction file uses the other factor-two convention for identical reactants and loads this class for species definitions only. | yes, ready |
